...
.controller('PostCtrl', function($scope, $ionicModal, $ionicPopup, $stateParams, DataLoader, WORDPRESS_API_URL, IONIC_APP_COLOR, LNG_POST_TITLE, LNG_POSTED, LNG_IN, LNG_COMMENTS_COUNT, LNG_NO_COMMENTS, LNG_FONT_ADJUST, LNG_ADD_COMMENT, LNG_TOAST_NO_DATA, LNG_SHARE_BUTTON, $ionicLoading, $sce, CacheFactory, $log, Bookmark, $timeout) {
    ...
    $scope.loadPost = function() {
        ...
        DataLoader.get(singlePostApi).then(function(response) {
            ...
            $scope.shownTab = response.data && response.data.tabs && response.data.tabs[0]; // set first tab active. #bluder
...
.controller('CategoriesCtrl', function($scope, $window, $http, DataLoader, $ionicLoading, $timeout, $ionicSlideBoxDelegate, WORDPRESS_API_URL, IONIC_APP_COLOR, $log) {
    $scope.color = IONIC_APP_COLOR;
	
    // Show All Categories
    var categoriesApi = WORDPRESS_API_URL + 'categories' + '?per_page=100&order=desc';
    ...
    $scope.loadCategories = function() {
        ...
        DataLoader.get(categoriesApi).then(function(response) {
            ...
            // each categories for check child categories. #bluder
    ...
    // Accordion list for subcategories. #bluder
    $scope.subGroup = [];
    $scope.toggleSubGroup = function(subGroup, event) {
